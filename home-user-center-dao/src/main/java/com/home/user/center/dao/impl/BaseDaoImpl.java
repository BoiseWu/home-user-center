...
import com.home.user.center.common.GlobalSessionField;
...
public abstract class BaseDaoImpl<T, ID extends Serializable> implements BaseDao<T, ID> {
	...
	public int insertBatch(List<T> tList){
		...
		int start = 0;
		int size = tList.size();
		int loop = size - GlobalSessionField.MAX_INSERT_ITEM;
		while(start < loop){
			insertCount += sqlTemplate.insert(sqlId,tList.subList(start,start + GlobalSessionField.MAX_INSERT_ITEM));
			start += GlobalSessionField.MAX_INSERT_ITEM;
		}
		if(start < size){
	...
	public Long selectCount(T t) {
		return sqlTemplate.selectOne(getNameSpace(SELECT_COUNT), t);
	}
}
